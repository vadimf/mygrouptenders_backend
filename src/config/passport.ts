    ...
    return asyncMiddleware(async (req: express.Request, res: express.Response, next: express.NextFunction) => {
        ...
        if ( authToken ) {
            ...
            if ( req.user ) {
                ...
                req.authToken = req.user.tokens.find((obj: IAuthenticationTokenDocument) => {
...
/**
 * Checks if administrator logged in, and then passes their object into request object. Otherwise - respond with an error.
 *
 * @returns <Promise<(req: e.Request, res: e.Response, next: e.NextFunction) => void>>
 */
